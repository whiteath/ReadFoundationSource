...
struct __CFRunLoopMode {
    ...
    uint64_t _timerSoftDeadline; /* TSR */      // SoftDeadline，timer触发的理想时间
    uint64_t _timerHardDeadline; /* TSR */      // HardDeadline，理想时间加上tolerance（偏差）
};
...
struct __CFRunLoop {
    ...
    // 该 Runloop 中加入到 CommonMode 中的Mode集合 （这是为了当新的Item加入到CommonMode中时，需要将这个Item同时加入到这些Modes中去）
    
    CFMutableSetRef _commonModeItems;
    // 这个集合里保存了所有加入到 CommonMode 的items (source, observer, timer)
    
    CFRunLoopModeRef _currentMode;
    CFMutableSetRef _modes;
    
    struct _block_item *_blocks_head;
    struct _block_item *_blocks_tail;       // runloop的 _block_item
...
// create 代表如果没有对应 modeName 的 runloop mode，则是否创建一个
// 所以这里包含了 创建 runloop mode 的过程
static CFRunLoopModeRef __CFRunLoopFindMode(CFRunLoopRef rl, CFStringRef modeName, Boolean create) {
    ...
    // event handler 被设置为更改 timerFiredPointer 指向的内容 rlm->_timerFired 为真！
    dispatch_source_set_event_handler(rlm->_timerSource, ^{
        *timerFiredPointer = true;
    });
    
    // 类似于设置NSTimer到FutureDistance一样，初始化为很远的时间，startTime 和 interval 均为DISPATCH_TIME_FOREVER
    ...
    // 将新创建的 queuePort 加入到 runloop mode 的 _portSet 集合中：
...
struct __CFRunLoopTimer {
    ...
    CFTimeInterval _tolerance;          /* mutable */       // 时间准确度的容忍度
...
// 取 rlt->_bits 的Bit 0，即 firing state
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    // 如果连 __CFRunLoops[字典，key为线程指针，value为runloop指针] 都没有，那么创建字典，创建主线程runloop
    if (!__CFRunLoops) {
        __CFUnlock(&loopsLock);
        CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, &kCFTypeDictionaryValueCallBacks);
        CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np());
    ...
    // 有了字典之后，访问字典获取当前线程的 runloop，如果没有，则创建
    ...
    //如果获取的 runloop 为当前线程的，则调用 SetTSD 赋值 （这样以后直接从TSD中就可以取到，而无需到 __CFRunLoops 字典里面取值了）
    
    if (pthread_equal(t, pthread_self())) {
        
        // _CFSetTSD 函数为设置 TSD 的 key 和 value 以及 构析函数
        _CFSetTSD(__CFTSDKeyRunLoop, (void *)loop, NULL);
        
        // 如果 __CFTSDKeyRunLoopCntr key 的 TSD 无内容，则设置。
        // 这里主要是设置 runloop 的构析函数 (即最后一个函数指针） ，以便在线程销毁的时候销毁runloop
...
CFRunLoopRef CFRunLoopGetCurrent(void) {
    ...
    // _CFGetTSD 为从TSD中获取 key 为 __CFTSDKeyRunLoop 的函数，这里 key 为 __CFTSDKeyRunLoop
    
    CFRunLoopRef rl = (CFRunLoopRef)_CFGetTSD(__CFTSDKeyRunLoop);
    
    // 获取到则返回，获取不到则去调用 _CFRunLoopGet0 （会创建）
...
// 执行 rl 中的所有 block items（以双向链表的方式储存）：
...
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {	/* DOES CALLOUT */
    ...
    // 至此，collectedObservers 存储了与该 activity 对应的 CFRunLoopObserverRef
...
static void __CFRunLoopCollectSources0(const void *value, void *context) {
    ...
    if (0 == rls->_context.version0.version && __CFIsValid(rls) && __CFRunLoopSourceIsSignaled(rls)) {
        ...
        // 如果sources为空（即参数里的context），说明是第一次调用该函数
        // 那么直接将 rls （也就是参数中的value，是一个SOURCE0）赋值给 sources 指向的内存即可
        if (NULL == *sources) {
            *sources = CFRetain(rls);
        } else if (CFGetTypeID(*sources) == CFRunLoopSourceGetTypeID()) {
            // 如果 sources 的类型为 CFRunLoopSource，说明是第二次调用该函数
            // 那么将 sources 改为一个 CFArray，并把原来的 CFRunLoopSource 和 这次的 CFRunLoopSource 都加入进来
...
static Boolean __CFRunLoopDoSources0(CFRunLoopRef rl, CFRunLoopModeRef rlm, Boolean stopAfterHandle) {	/* DOES CALLOUT */
    ...
    if (NULL != rlm->_sources0 && 0 < CFSetGetCount(rlm->_sources0)) {
        ...
        // 将 rlm->_sources0 里的 CFRunLoopSource 都拷贝到 sources 指向的内存里
    ...
    if (NULL != sources) {
        ...
        if (CFGetTypeID(sources) == CFRunLoopSourceGetTypeID()) {
            // 如果 sources 只是一个单独的 CFRunLoopSourceRef：
            CFRunLoopSourceRef rls = (CFRunLoopSourceRef)sources;
            __CFRunLoopSourceLock(rls);
            // 如果是被标记为 Signaled，就开始执行
            ...
            // 如果 sources 是一个 CFRunLoopSourceRef 的数组：
            ...
            for (CFIndex idx = 0; idx < cnt; idx++) {
                ...
                // 如果参数 stopAfterHandle 为true，处理过一个之后 就break
...
static Boolean __CFRunLoopDoTimers(CFRunLoopRef rl, CFRunLoopModeRef rlm, uint64_t limitTSR) {	/* DOES CALLOUT */
    ...
    for (CFIndex idx = 0, cnt = rlm->_timers ? CFArrayGetCount(rlm->_timers) : 0; idx < cnt; idx++) {
        ...
        // 如果此 timer 有效 && 其 firing state 为Firing && _fireTSR < limitTSR {疑问？}
...
// __CFRunLoopServiceMachPort 接收端口或端口集合的消息，其中会调用 mach_msg 来让线程进行休眠
static Boolean __CFRunLoopServiceMachPort(mach_port_name_t port, mach_msg_header_t **buffer, size_t buffer_size, mach_port_t *livePort, mach_msg_timeout_t timeout, voucher_mach_msg_state_t *voucherState, voucher_t *voucherCopy) {
    ...
    for (;;) {		/* In that sleep of death what nightmares may come ... */
        ...
        // 调用 mach_msg，传递 MACH_RCV_MSG 表明 接收消息，传递 port 表示接收的端口，传递 timeout 为超时时间
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
...
#if __HAS_DISPATCH__  // 如果支持GCD的话
    
    __CFPort dispatchPort = CFPORT_NULL;
    
    Boolean libdispatchQSafe = pthread_main_np() && ((HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && NULL == previousMode) || (!HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && 0 == _CFGetTSD(__CFTSDKeyIsInGCDMainQ)));
    
    // 仅当当前当前线程为主线程，且第一次进入 Runloop 时执行，且当前 Mode 在 CommonModes 中
    if (libdispatchQSafe && (CFRunLoopGetMain() == rl) && CFSetContainsValue(rl->_commonModes, rlm->_name))
        dispatchPort = _dispatch_get_main_queue_port_4CF();   //// 从 GCD 的私有 API 获取一个队列调度端口（4CF 表示 for Core Foundation）
#endif
    
    // 如果使用 GCD timer 作为 timer 的实现，在此进行准备工作
#if USE_DISPATCH_SOURCE_FOR_TIMERS
    ...
    // ** 设置 runloop 自身的超时，回调函数为：__CFRunLoopTimeout
#if __HAS_DISPATCH__
    ...
    do {
        ...
        // 给waitSet赋值：当前mode中需要监听的 port 集合
        __CFPortSet waitSet = rlm->_portSet;
        
        __CFRunLoopUnsetIgnoreWakeUps(rl);
        
        // __CFRunLoopDoObservers 用来通知 Observers 并执行其中包含的 _callout：
        ...
        // 返回值 sourceHandledThisLoop 指的是 是否有Sources0（被处理），如果 rlm 一个 source0 都没有，就返回false；如果至少有一个，就返回true。
        
        // 如果有过Source0，则再次 DoBlock  {疑问：why？}
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm);
        }
        
        
        // poll 指的 Sources0 是否被处理过，只要有一个处理过，即为真
        Boolean poll = sourceHandledThisLoop || (0ULL == timeout_context->termTSR);
        
        
        
        // *** 这一部分是从 dispatchPort（若存在的话） 中去取消息，取到消息则跳转到 handle_msg
        // 第一次进入runloop不会去执行，didDispatchPortLastTime为真不会执行
        ...
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
            ...
            // 使用 __CFRunLoopServiceMachPort 接受 dispatchPort 的消息
            // 如果成功，消息保存在 msg 中，跳转到 handle_msg
            // 倒数第三个参数 timeout 为超时时间，这里设置为0，则表示此刻取不到消息立即返回，不休眠
        ...
        didDispatchPortLastTime = false;  // didDispatchPortLastTime 设为 false
        

        // 如果 poll 为 false（即如果之前没处理过 Sources0），就会通知线程进入睡眠。
        // 后续可以看到如果 poll 为 true ，就不休眠
        if (!poll && (rlm->_observerMask & kCFRunLoopBeforeWaiting))
            __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeWaiting);
        
        
        // 设置 runloop 的标识（ _cfinfo[CF_INFO_BITS] ）为 sleep，方便查询 Runloop 的状态
        ...
        // 将 dispatchPort 也加入到 waitSet 里面去
#if __HAS_DISPATCH__
        ...
        do {
            ...
            // 再次调用 __CFRunLoopServiceMachPort
            // 注意如果 poll 为 false，那么 timeout 会设置为 TIMEOUT_INFINITY，表示会休眠  { 疑问：为什么poll为true，也就是处理过Sources0的话，就不休眠呢？ }
            // [这里就是 runloop 睡觉的时刻了]
            
            // 若取到了消息，就会退出休眠。然后消息会保存在 msg，livePort 会保留 msg 的端口，也就是收到消息的端口
            __CFRunLoopServiceMachPort(waitSet, &msg, sizeof(msg_buffer), &livePort, poll ? 0 : TIMEOUT_INFINITY, &voucherState, &voucherCopy);
            
            
            // 如果接收消息的是 modeQueuePort，说明是来自 CGD 实现的Timer {  疑问：这里没看懂 }
        ...
        // 计算睡了多少时间，并加到runloop里面
        rl->_sleepTime += (poll ? 0.0 : (CFAbsoluteTimeGetCurrent() - sleepStart));
        
        // 将 dispatchPort 从 waitSet 里面移除。解释 为何要不断移除和加入：
        ...
        __CFRunLoopSetIgnoreWakeUps(rl);     //（疑问：这是干什么？）
        
        // user callouts now OK again
        __CFRunLoopUnsetSleeping(rl);
        
        
        // 如果之前睡过，就向 observer 发送 kCFRunLoopAfterWaiting 消息
    ...
    handle_msg:;
        ...
        if (MACH_PORT_NULL == livePort) {
            ...
            // 外部程序调用了 CFRunLoopWakeUp 主动唤醒
            
            CFRUNLOOP_WAKEUP_FOR_WAKEUP();
            // do nothing on Mac OS
        }
        
        // 如果使用GCD来实现 timer，那这里就是被 timer 唤醒的，执行 timers
#if USE_DISPATCH_SOURCE_FOR_TIMERS
...
#endif
        // 如果使用 MK_TIMER 来实现 timer，那这里就是被 timer 唤醒的，执行 timers
            ...
            // dispatchPort 发来的消息，表明是GCD异步方法的唤醒
            // gcd dispatch到main_queue的block，执行block，即 dispatch_async(dispatch_get_main_queue(), block) 产生的任务
            // 这个逻辑仅限于 dispatch 到主线程，dispatch 到其他线程仍然是由 libDispatch 处理的
            ...
            // 处理 消息，最终调用了 _dispatch_main_queue_callback_4CF
        ...
        // 被 Source1 唤醒
            ...
            if (rls) {
                ...
                if (NULL != reply) {
                    ...
                    // reply 是执行了 source1 的回调的返回值，即如果返回值不为空，则将返回值再通过 Mach 消息的方式传递回去：
        ...
        if (sourceHandledThisLoop && stopAfterHandle) {
            // 如果处理过 source0 或 timer 或 source1，且 stopAfterHandle 为true，则返回 kCFRunLoopRunHandledSource，结束runloop
            retVal = kCFRunLoopRunHandledSource;
        } else if (timeout_context->termTSR < mach_absolute_time()) {
            // runloop 超时，返回 kCFRunLoopRunTimedOut，结束runloop
            ...
            // runloop mode 为空，返回 kCFRunLoopRunFinished，结束runloop
...
void CFRunLoopPerformBlock(CFRunLoopRef rl, CFTypeRef mode, void (^block)(void)) {
    ...
    if (__CFMainThreadHasExited && rl == CFRunLoopGetMain()) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            CFLog(kCFLogLevelError, CFSTR("Attempting to perform block on main runloop, but the main thread as exited. This message will only log once. Break on _CFRunLoopError_MainThreadHasExited to debug."));
        });
...
void CFRunLoopAddSource(CFRunLoopRef rl, CFRunLoopSourceRef rls, CFStringRef modeName) {	/* DOES CALLOUT */
    ...
    if (modeName == kCFRunLoopCommonModes) {
        ...
        if (NULL != set) {
            ...
            // 如果 modeName 为 Common 的话，就需要把这个itme（此处是Source）加入到Comom下的所有modes中
        ...
        if (NULL != rlm && NULL == rlm->_sources0) {
            // 如果 rlm 的 sources0 还为空，则创建 _sources0 和 _sources1 连个 CFSet 以及一个 _portToV1SourceMap 一个 CFDictionary
            // _portToV1SourceMap 为一个字典，key为port，value 为 source1，是为了在runloop主逻辑中过滤runloop自己的port消息
        ...
        if (NULL != rlm && !CFSetContainsValue(rlm->_sources0, rls) && !CFSetContainsValue(rlm->_sources1, rls)) {
            ...
            // 将rls保存在对应的rlm的source0或者source1
            ...
            if (0 == rls->_context.version0.version) {
                ...
                if (CFPORT_NULL != src_port) {
                    //如果是source1，则同时更新 _portToV1SourceMap 以及将 src_port 加入到 rlm->_portSet 中去
            ...
            __CFRunLoopSourceLock(rls);
            if (NULL == rls->_runLoops) {
                // CFBag 基本上等同于 NSCountedSet，不过不是 total bridge 的
    ...
    // 如果 此 source0 的 schedule 不为空，则在此执行 schedule 回调
...
void CFRunLoopAddTimer(CFRunLoopRef rl, CFRunLoopTimerRef rlt, CFStringRef modeName) {
    ...
    if (modeName == kCFRunLoopCommonModes) {
        // 如果是加到 kCFRunLoopCommonModes 中去：
        ...
        if (NULL != set) {
            ...
            // 将新的 CFRunLoopTimer 加到 set 里面的每个 _commonModes 中去（通过 执行 __CFRunLoopAddItemToCommonModes 操作）
            CFSetApplyFunction(set, (__CFRunLoopAddItemToCommonModes), (void *)context);
            CFRelease(set);
        }
    } else {
