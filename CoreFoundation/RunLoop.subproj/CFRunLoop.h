...
typedef struct {
    ...
    void	(*schedule)(void *info, CFRunLoopRef rl, CFRunLoopMode mode);   // source0 被加入到runloop时的回调
    void	(*cancel)(void *info, CFRunLoopRef rl, CFRunLoopMode mode);     // source0 被取消时的回调
    void	(*perform)(void *info);                                         // source0 所包含的执行回调
